<template>
    <div class="todo-header">
      ...
      <!-- 方式二对应的写法,由于需要从input框收集用户输入数据,绑定到data的title上,所以必须用v-model,
        如果用v-bind,只能是内部影响到页面,用户的输入不会改变title值!  具体为什么需要title,这里就不细说了!
        -->
...
export default {
  name: 'TodoHeader',
  // 方式一：需要用这个props去接收从App组件传过来的的函数addTodo。注意,方式一addTodo是函数,而
  //        对于方式二,addTodo是App组件给TodoHeader绑定的自定义事件了。
  // props: ['addTodo'],

  // 这是对应方式二,由于使用v-model="title",双向绑定title, 需要data配置项里的title,
  // 所以methods对应的方式二的add,可以通过this.title直接获取到用户输入的数据
  data() {
    return {
      title: ''
    }
  ...
  methods: {
    ...
    // 方式一：  addTodo是函数!!! 所以是用来调用！！！
    // add(e){     //这个e是指: 按回车键确认" @keyup.enter="add" 这个键盘事件的对象。
    //   // 将用户的输入包装成一个todo对象：需要获取用户输入值 title, 由于没有使用Vue的数据绑定v-model,所以通过e.target.value来获取用户输入。

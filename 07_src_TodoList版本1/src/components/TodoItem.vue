<template>
  <li>
    <label>
      ...
      <!-- 但是不建议用v-model,因为todo是props传过来的,props的值是不可以修改的,但是由于props是浅监视,所以对于复杂类型的属性改了,vue检测不到props的变化,但是如果props传过来的数据是简单类型,那么就会报错了!!! -->
      <!-- 另外,:checked="todo.done",通过todo.done的布尔值来决定checked是否出现,如果出现,那么就勾选,反之,不勾选 -->
      <!-- v-model="todo.done" 数据双向绑定,而且因为todo.done的值刚好是布尔值,正好可以决定checked是否出现,checked出现,单选框打勾,反之单选框不打勾 -->
